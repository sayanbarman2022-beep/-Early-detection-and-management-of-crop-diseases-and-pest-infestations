...
// Then ensure ${audioHtml} is injected inside the card.innerHTML just under${notesHtml}
  ...
  filtered.forEach(c=>{
    ...
    // Build HTML for the missing report parts
    ...
    card.innerHTML = `
      ...
      <div class="case-body">
        ...
        <!-- Newly Injected Notes, Extra Images, and Audio -->
        ${farmerNotesHtml}
        ${extraImagesHtml}
        ${audioHtml}
...
document.getElementById("soilInput").addEventListener("change", (e) => {
  ...
  reader.onload = () => {
    ...
    document.getElementById("soilPreview").innerHTML = `<img src="${soilDataUrl}" alt="Soil Condition"><span class="badge-attached">Soil Added</span>`;
  };
  reader.readAsDataURL(file);
});

// Fruit/Vegetable image upload handler
document.getElementById("fruitInput").addEventListener("change", (e) => {
  ...
  reader.onload = () => {
    ...
    document.getElementById("fruitPreview").innerHTML = `<img src="${fruitDataUrl}" alt="Fruit Condition"><span class="badge-attached">Fruit Added</span>`;
  };
  reader.readAsDataURL(file);
});
// Add these to your global state variables
let mediaRecorder;
let audioChunks = [];
  ...
  if (isRecording) {
    ...
    try {
      ...
      mediaRecorder.addEventListener("stop", () => {
        ...
        // Convert Blob to Base64 to attach to the JSON payload
        const reader = new FileReader();
        ...
        reader.onloadend = () => {
          ...
          if (recordBtn) recordBtn.textContent = "Record New Note";
        };
      });
...
if (removeAudioBtn) {
  removeAudioBtn.addEventListener("click", () => {
    ...
    if (recordBtn) recordBtn.textContent = "Record Audio Note";
  });
}

// REMEMBER: Update your onReferClick() function to include audioDataUrl in the caseObj!
// audioNoteDataUrl: audioDataUrl || null,
initApp();